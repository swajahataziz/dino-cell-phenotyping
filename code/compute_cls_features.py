...
def extract_and_save_feature_pipeline(args):
    ...
    if not args.images_are_RGB:
        ...
        print('Creating dataset with MultiChannelDataset')
        '''

        dataset_total =  MultiChannelDataset('/fsx/data/images', transform=transform)    
    ...
    print("Image shape: "+str(local_batch[0].shape))
    ...
    print(f"Lables found: {local_lables}")
    ...
    labels= [dataset_total.samples[i][1] for i in index_all]

    class_to_idx = dataset_total.class_to_idx
    idx_to_class = {v: k for k, v in class_to_idx.items()}

    label_names = [idx_to_class[l] for l in labels]
    
    
    #labels = [utils.fetch_foldername_of_img_location(dataset_total,i, args.folder_depth_for_labels) for i in index_all]
    return features, label_names, image_names
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_weights', default='/fsx/data/checkpoint.pth', type=str, help="Path to pretrained weights to evaluate.")
    ...
    parser.add_argument('--selected_channels', default=[0], type=list, help="""list of channel indexes of the .tiff images which should be used to create the tensors.""")
    parser.add_argument('--channel_dict', default="Brightfield", help="""name of the channels in format as dict channel_number, channel_name.""")
    ...
    if len(list(args.selected_channels)) >1:
    ...
    if dist.get_rank() == 0:
        ...
        np.savetxt(f"{args.output_dir}/{epoch_path}_features.csv", features.cpu().numpy(), delimiter=",")
        with open(os.path.join(args.output_dir, "run_log.txt"), "w") as f:
            f.write(f"computing features with seed {args.seed}: \n")
            f.write("parameters: \n")
            for arg in vars(args):
                f.write(f"{arg} : {getattr(args, arg)} \n")
        print(f"Saving labels to {cls_directory}")
        np.savetxt(f"{args.output_dir}/{cls_directory}/class_labels.csv", labels, delimiter=",", fmt="%s")
        np.savetxt(f"{args.output_dir}/{cls_directory}/image_paths.csv", image_names, delimiter=",", fmt="%s")
