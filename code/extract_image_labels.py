...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize_length', default=224, help="""quadratic resize length to resize images""")
    ...
    if dist.get_rank() == 0:
        ...
        if args.scDINO_full_pipeline:
            path = os.path.join(args.output_dir,args.name_of_run)+"/"
             ...
             path = os.path.join('/fsx/data', args.name_of_run, f"CLS_features/")
        path = os.path.join("/fsx/data", args.name_of_run, f"CLS_features/")
        print("Saving files to path: "+ path)
        np.savetxt(f"/fsx/data/output/class_labels.csv", labels, delimiter=",", fmt="%s")
        np.savetxt(f"/fsx/data/output/image_paths.csv", image_names, delimiter=",", fmt="%s")
